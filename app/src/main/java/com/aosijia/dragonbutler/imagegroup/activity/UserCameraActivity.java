...
public class UserCameraActivity extends AppCompatActivity {
    ...
    private final String EXTRA_PHOTO_URL = com.aosijia.dragonbutler.imagegroup.NavigatorImage.EXTRA_PHOTO_URL;
    ...
    private void recoverFile(String url) {
        File mFile = new File(com.aosijia.dragonbutler.imagegroup.utils.ImageGroupUtils.getPathOfPhotoByUri(this, Uri.parse(url)));
        try {
            if (com.aosijia.dragonbutler.imagegroup.utils.FileUtils.fileIsAvaliableImage(mFile)) {
    ...
    private void finishAfterDelete(String url) {
        String path = new File(ImageGroupUtils.getPathOfPhotoByUri(this, Uri.parse(url))).getAbsolutePath();
        System.out.println(path);
        com.aosijia.dragonbutler.imagegroup.utils.FileUtils.deleteFile(new File(com.aosijia.dragonbutler.imagegroup.utils.ImageGroupUtils.getPathOfPhotoByUri(this, Uri.parse(url))));
        finish();
    }

    private void finishWithResult(String url) {
        Intent rsl = new Intent();
        rsl.putExtra(EXTRA_PHOTO_URL, com.aosijia.dragonbutler.imagegroup.utils.ImageGroupUtils.getPathOfPhotoByUri(this, Uri.parse(url)));
