 ...
 *******************************************************************************/
package com.aosijia.dragonbutler.ui.widget.pulllistview;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if(Math.abs(event.getRawY() - mLastY)>10) {
    ...
    public void onLoadMoreComplete(boolean hasMore) {
        if (!hasMore) {
            getLoadingLayoutProxy(false, true).setReleaseLabel("没有更多了");
            getLoadingLayoutProxy(false, true).setPullLabel("没有更多了");
            getLoadingLayoutProxy(false, true).setRefreshingLabel("没有更多了");
	...
	protected void onPullToRefresh() {
		...
		case PULL_FROM_START:
			if(!mIsInvisible){
//				long lastUpdateTime = Preferences.getLastNewsTime(mContext,
//                        channelId);
//				String time = TimeUtils.converTime(lastUpdateTime);
//				getLoadingLayoutProxy().setLastUpdatedLabel(time + "前更新");
//				mHeaderLayout.pullToRefresh(time);
			}
	...
	public void setHideSubText(boolean mIsInvisible){
		this.mIsInvisible = mIsInvisible;
	}
	
	public void setSubHeaderTextVisible(int visible){
		mHeaderLayout.setSubHeaderTextVisible(visible);
	}
	...
	public void setOnSlideListener(OnSlideLisnter listener){
		mOnSlideListener = listener;
	}

	private OnSlideLisnter mOnSlideListener;
	private float mLastY;

	public interface OnSlideLisnter {
		void onSlide();
	}
