...
public class Kanner extends FrameLayout {
    ...
    public void setImagesUrl(Object[] imagesUrl) {
        initLayout();
        initImgFromNet(imagesUrl,-1);
        showTime();
    }


    public void setImagesUrl(Object[] imagesUrl,int defaultImgResId) {
        initLayout();
        initImgFromNet(imagesUrl,defaultImgResId);
        showTime();
    }
    ...
    private void initImgFromRes(int[] imagesRes) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            iv.setImageResource(R.drawable.default_pic_details);
//            iv.setBackgroundResource(R.drawable.loading);
    ...
    private void initImgFromNet(Object[] imagesUrl,int defaultImageResId) {
...
//            iv.setBackgroundResource(R.mipmap.loading);
            if (i == 0) {
                mImageLoader.displayImage(imagesUrl[count - 1].toString(), iv, options);
            } else if (i == count + 1) {
                mImageLoader.displayImage(imagesUrl[0].toString(), iv, options);
            } else {
                mImageLoader.displayImage(imagesUrl[i - 1].toString(), iv, options);
            }
            imageViews.add(iv);
        }
    }
